...
**Watchlist names, during US market hours: just read the URL.** The intraday
fetch runs every 30 minutes over the watchlist — at :03 and :33, 13:00–19:59 UTC
on weekdays — so
`https://raw.githubusercontent.com/jhirgit/daily-prices/main/data/intraday.json`
is at most ~35 minutes stale. Check `as_of` to see how fresh it is.

Two caveats on that URL. It needs **no auth today only because this repo is
PUBLIC**; jr-dash backlog #15 flips it private, after which the raw URL stops
answering and the board's `/data/` proxy is the only path (see README §1).
And the schedule is the `cron-dispatch` Cloudflare Worker, not a GitHub cron:
GitHub's own schedule block was deleted from this workflow on 2026-09-13 because
its late deliveries were overwriting the frozen pre-close mark. If the data
looks stuck, check `curl -s
https://jr-cron-dispatch.jakeradencom.workers.dev/heartbeat` before assuming the
fetch is broken.

**Arbitrary tickers, or fresher than that: dispatch the workflow.** Requires
